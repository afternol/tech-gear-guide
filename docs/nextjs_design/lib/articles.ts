...
export async function getMustCatchArticles(limit = 5): Promise<ArticleSummary[]> {
  ...
  if (error) throw new Error(`getMustCatchArticles: ${error.message}`)
  return (data ?? []) as ArticleSummary[]
}
...
> {
  const categories: Category[] = ['smartphone', 'tablet', 'windows', 'cpu_gpu', 'ai', 'general']
  const db = createServerClient()
  ...
  return Object.fromEntries(
    categories.map((cat, i) => [cat, (results[i].data ?? []) as ArticleSummary[]])
